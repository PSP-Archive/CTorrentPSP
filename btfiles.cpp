...
int btFiles::_btf_ftruncate(int fd,int64_t length)
{
  ...
  int retval = -1;//FIXME: ftruncate(fd,length);
  if( retval < 0 ) {
    char c = (char)0;
    if( lseek(fd,length - 1, SEEK_SET) < 0 ) return -1;
    return write(fd, &c, 1);
    ...
    if(stat(sp,&sb) < 0){
      if( ENOENT == errno ){
        if( !last ){
          ...
          if((fd = open(sp, O_WRONLY|O_CREAT|O_TRUNC, 0644)) < 0){ last = 0; break; }
...
{
  //PSPNOTE: This can be used to get the file list and completion percentage!
  BTFILE *p = m_btfhead;
  size_t id = 0;
  CONSOLE.Print("");
...
void btFiles::SetFilter(int nfile, BitField *pFilter, size_t pieceLength) const
{
  BTFILE *p = m_btfhead;
  size_t id = 0;
...
int btFiles::GetFileCompletionPercent(size_t nfile) const
{
	BitField tmpBitField, tmpFilter;
	BTCONTENT.SetTmpFilter(nfile, &tmpFilter);
	tmpBitField = *BTCONTENT.pBF;
	tmpBitField.Except(tmpFilter);
	
	int filepieces = GetFilePieces(nfile);
	if (filepieces > 0)
		return 100 * tmpBitField.Count() / filepieces;
	else
		return 100;
}
