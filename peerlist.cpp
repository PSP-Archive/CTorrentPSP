    ...
    if( MIN_UNCHOKE_INTERVAL <= now - m_interval_timestamp ){
      ...
      if( !BandWidthLimitUp() ||
          ( m_prev_limit_up &&
            ...
            ( cfg_max_bandwidth_up < (int)cfg_req_slice_size * (MIN_OPT_CYCLE-1) /
                                     (MIN_UNCHOKE_INTERVAL * MIN_OPT_CYCLE) ||
              m_prev_limit_up < (int)cfg_req_slice_size * (MIN_OPT_CYCLE-1) /
                                (MIN_UNCHOKE_INTERVAL * MIN_OPT_CYCLE) ) ) ){
        SetUnchokeIntervals();
      }
  ...
  int f_unchoke_check, btPeer **UNCHOKER)
{
 ...
 again:
  ...
  if( f_unchoke_check && UNCHOKER ){
    ...
    for( unsigned int i = 0; i < m_max_unchoke + 1; i++ ){
      if( !UNCHOKER[i] ) break;
...
// The m_max_unchoke+1 (4th) slot is for the optimistic unchoke when it happens.
  ...
  for( cancel_idx = i = 0; i < (int)(m_max_unchoke + no_opt); i++ ){
